...
def interpolate_poses_with_timestamps(poses: torch.Tensor, times: torch.Tensor) -> torch.Tensor:
    """
    Interpolate SE(3) poses at given normalized time stamps ∈ [0, 1].

    Args:
        poses (torch.Tensor): (N, 4, 4) input poses.
        times (torch.Tensor): (M,) normalized time stamps in [0, 1].

    Returns:
        torch.Tensor: (M, 4, 4) interpolated poses.
    """
    assert poses.ndim == 3 and poses.shape[1:] == (4, 4)
    assert times.ndim == 1
    assert torch.all((0 <= times) & (times <= 1)), "Time stamps must be in [0, 1]"

    N = poses.shape[0]
    M = times.shape[0]
    device, dtype = poses.device, poses.dtype

    # === Convert to index space [0, N-1]
    pose_times = times * (N - 1)
    t0_idx = torch.floor(pose_times).long()
    t1_idx = torch.clamp(t0_idx + 1, max=N - 1)
    alpha = (pose_times - t0_idx.float()).unsqueeze(1)  # (M, 1)

    # === Translation interpolation (GPU)
    t_all = poses[:, :3, 3]  # (N, 3)
    t_interp = (1 - alpha) * t_all[t0_idx] + alpha * t_all[t1_idx]  # (M, 3)

    # === Rotation interpolation (SLERP on CPU)
    poses_np = poses.detach().cpu().numpy()
    key_rots = R.from_matrix(poses_np[:, :3, :3])
    slerp = Slerp(np.arange(N), key_rots)
    R_interp = slerp(pose_times.detach().cpu().numpy())  # returns Rotation object

    # === Assemble final SE(3)
    result = torch.eye(4, device=device, dtype=dtype).unsqueeze(0).repeat(M, 1, 1)
    result[:, :3, :3] = torch.tensor(R_interp.as_matrix(), dtype=dtype, device=device)
    result[:, :3, 3] = t_interp
