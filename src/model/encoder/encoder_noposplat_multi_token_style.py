...
from .encoder_noposplat_token_style import EncoderNoPoSplatTokenStyleCfg
...
from .token_stylizer.token_stylizer import TokenStylizer
...
class EncoderNoPoSplatMultiTokenStyle(Encoder[EncoderNoPoSplatTokenStyleCfg]):
    backbone: nn.Module
    ...
    def __init__(self, cfg: EncoderNoPoSplatTokenStyleCfg) -> None:
        ...
        # init token_stylizer and gs_sh head
        self.stylized = cfg.stylized
        self.token_stylizer = TokenStylizer(cfg.token_stylizer)
        self.token_stylizer.depth_mode = ('exp', -inf, inf)
        self.token_stylizer.conf_mode = None
    ...
    def set_gs_params_head(self, cfg, head_type):
        if head_type == 'linear':
            ...
            self.gaussian_param_head = head_factory(head_type, 'gs_params', self.backbone, has_conf=False, out_nchan=self.raw_gs_dim - 3 * self.gaussian_adapter.d_sh)
            self.gaussian_param_head2 = head_factory(head_type, 'gs_params', self.backbone, has_conf=False, out_nchan=self.raw_gs_dim - 3 * self.gaussian_adapter.d_sh)
        else:
    ...
    def forward(
        ...
        enc_feat, enc_pose, dec_feat, shape, images = self.backbone(context)
        sty_feat = self.token_stylizer(style, enc_feat, enc_pose)
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            # for appearance head
            for i in range(v):
                GS_appearance = self.gaussian_appearance_head([tok[:, i].float() for tok in sty_feat], shape[0, i].cpu().tolist())
                GS_appearance = rearrange(GS_appearance, "b d h w -> b (h w) d")
                all_gs_appearances.append(GS_appearance)
        ...
        gaussians_appearance = torch.stack(all_gs_appearances, dim=1)
        
        gaussians = torch.cat((gaussians[...,:self.raw_gs_dim - 3 * self.gaussian_adapter.d_sh], gaussians_appearance), dim=-1)
