...
class PatchEmbedDust3R(PatchEmbed):
    def forward(self, x, **kw):
        ...
        x = self.proj(x)  # comment: conv: [4,3,256,256]->[4,1024,16,16]
        pos = self.position_getter(B, x.size(2), x.size(3), x.device)  # com: position of every 16x16, [[0,0], [0,1],..., [15, 15]]
        if self.flatten:
            x = x.flatten(2).transpose(1, 2)  # BCHW -> BNC
        x = self.norm(x)  # com: nn.Identity()
        return x, pos
