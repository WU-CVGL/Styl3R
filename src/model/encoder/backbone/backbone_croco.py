...
class AsymmetricCroCo(CroCoNet):
    ...
    def __init__(self, cfg: BackboneCrocoCfg, d_in: int) -> None:
        ...
        if cfg.asymmetry_decoder:
            # here: the weights of dec_blocks2 & dec_blocks are not same
    ...
    def _decoder(self, f1, pos1, f2, pos2, extra_embed1=None, extra_embed2=None):
        ...
        f1 = self.decoder_embed(f1)  # com: linear
        f2 = self.decoder_embed(f2)  # com: linear

        final_output.append((f1, f2))
        for blk1, blk2 in zip(self.dec_blocks, self.dec_blocks2):
            # img1 side (f1, f2, pos1, pos2)
            f1, _ = blk1(*final_output[-1][::+1], pos1, pos2)
            # img2 side (f2, f1, pos2, pos1)
    ...
    def forward(self,
        ...
        if return_views:
            if return_feats:
                return feat1, pos1, feat2, pos2, dec1, dec2, shape1, shape2, view1, view2
            else:
