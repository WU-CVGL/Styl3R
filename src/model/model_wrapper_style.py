...
from ..loss.loss_identity import IdentityLoss
...
from ..test.vgg_model import AdaIN2D
...
class ModelWrapperStyle(LightningModule):
    ...
    def __init__(
        ...
        if self.distiller is not None:
            ...
            self.distiller_loss = Regr3D(norm_mode=None)
        ...
        # 2D AdaIN model as baseline
        if self.global_rank == 0:
            self.AdaIN2D = AdaIN2D()
            self.AdaIN2D.load_state_dict(torch.load('ckpts/model_state.pth', map_location='cpu'))
            convert_to_buffer(self.AdaIN2D, persistent=False)
    ...
    def training_step(self, batch, batch_idx):
            ...
            for batch_per_dl in batch:
                if batch_combined is None:
                    ...
                    for k in batch_combined.keys():
                        if k == 'style':
                            batch_combined[k]['image'] = torch.cat([batch_combined[k]['image'], batch_per_dl[k]['image']], dim=0)
                            batch_combined[k]['image_name'] += batch_per_dl[k]['image_name']
                        else:
        ...
        # apply normalization to context images: scale img within [-1, 1]
        batch: BatchedExample = self.data_shim(batch)
        # however target images are still within [0, 1]
        ...
        if not self.encoder.stylized:
            style = {'image': batch["context"]["image"][:, 0]}
        else:
            style = batch["style"]
            style = batch["style"].copy()  # shallow copy of the dictionary
            style['image'] = style['image'].clone()  # clone the tensor to prevent modification of original
            style['image'] = (style['image'] - 0.5) / 0.5 # (0, 1) -> (-1, 1)
                
        if self.distill_only:
            self.encoder(batch["context"], style, self.global_step, visualization_dump=visualization_dump, distill_only=self.distill_only)
            ...
            if self.distiller is not None:
                ...
                distillation_loss = self.distiller_loss(pseudo_gt1['pts3d'], pseudo_gt2['pts3d'],
                                                        ...
                                                        pseudo_gt1['conf'], pseudo_gt2['conf'], disable_view1=False)
            ...
            gaussians = self.encoder(batch["context"], style, self.global_step, visualization_dump=visualization_dump)
            ...
            if self.identity_loss is not None:
                identity_style = {'image': batch["context"]["image"][:, 0]}
                identity_gaussians = self.encoder(batch["context"], identity_style, self.global_step, visualization_dump=visualization_dump)
                identity_output = self.decoder.forward(
                    identity_gaussians,
                    batch["target"]["extrinsics"],
                    batch["target"]["intrinsics"],
                    batch["target"]["near"],
                ...
                identity_loss = self.identity_loss(identity_output, batch, identity_gaussians, self.global_step)
                self.log("loss/identity_loss", identity_loss)
        ...
        # log image every 500 steps
        if (self.global_rank == 0 
            and self.global_step % (50 * self.train_cfg.print_log_every_n_steps) == 0):

            comparisons = []
            captions = []

            for index in [0, -1]:  # first and last sample
                rgb_gt = target_gt[index]
                rgb_pred = output.color[index]
                depth_pred = vis_depth_map(output.depth[index])
                
                context_img = inverse_normalize(batch["context"]["image"][index])
                style_img = batch["style"]["image"][index]
                context = []
                for i in range(context_img.shape[0]):
                    context.append(context_img[i])
                    context.append(style_img)
                
                # stylize target views with 2D AdaIN as baseline
                with torch.no_grad():
                    rgb_gt_vgg = self.vgg_normalize(rgb_gt)
                    style_img_vgg = self.vgg_normalize(style_img)
                    style_img_vgg = style_img_vgg.unsqueeze(0).repeat(rgb_gt_vgg.shape[0], 1, 1, 1)
                    stylized_rgb_gt = self.AdaIN2D.generate(rgb_gt_vgg, style_img_vgg, 1)
                    stylized_rgb_gt = vgg_denorm(stylized_rgb_gt)
                baseline = [stylized_rgb_gt[i] for i in range(stylized_rgb_gt.shape[0])]

                comparison = hcat(
                    add_label(vcat(*baseline), "2D Baseline"),
                ...
                comparison = add_label(comparison, f"Sample Index: {index}")
                comparisons.append(comparison)
                captions.append(f"{batch['scene'][index]} | {batch['style']['image_name'][index]}")

            final_image = hcat(*comparisons)  # stack first and last vertically
            final_caption = " | ".join(captions)

            self.logger.log_image(
                "train/comparison",
                [prep_image(add_border(final_image))],
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if not self.encoder.stylized:
            ...
            style = batch["style"].copy()  # shallow copy of the dictionary
            style['image'] = style['image'].clone()  # clone the tensor to prevent modification of original
            style['image'] = (style['image'] - 0.5) / 0.5
        ...
        with torch.no_grad():
            ...
            stylized_rgb_gt = vgg_denorm(stylized_rgb_gt)
        baseline = []
        for i in range(stylized_rgb_gt.shape[0]):
            baseline.append(stylized_rgb_gt[i])
        ...
        comparison = hcat(add_label(batch["style"]["image"][0], "Style"), comparison)
        caption = batch["scene"][0] + " | " + batch["style"]['image_name'][0]
        
        if self.identity_loss is not None:
            identity_style = {'image': batch["context"]["image"][:, 0]}
            identity_gaussians = self.encoder(batch["context"], identity_style, self.global_step, visualization_dump=None)
            identity_output = self.decoder.forward(
                identity_gaussians,
                batch["target"]["extrinsics"],
                batch["target"]["intrinsics"],
            ...
            identity_loss = self.identity_loss(identity_output, batch, identity_gaussians, self.global_step)
            self.log("val/identity_loss", identity_loss)
            identity_pred = identity_output.color[0]
            comparison = hcat(comparison, add_label(vcat(*identity_pred), "Identity (Prediction)"))
        ...
        if self.distiller is not None:
            ...
            distillation_loss = self.distiller_loss(pseudo_gt1['pts3d'], pseudo_gt2['pts3d'],
                                                    ...
                                                    visualization_dump['means'][:, 1].squeeze(-2),
                                                    pseudo_gt1['conf'], pseudo_gt2['conf'], disable_view1=False)
            self.log("val/distillation_loss", distillation_loss)
    ...
    def render_video_generic(
        ...
        gaussians = self.encoder(batch["context"], style, self.global_step)
    ...
    def configure_optimizers(self):
        # TODO: add logic to choose params to optimize when stylized
        new_params, new_param_names = [], []
        pretrained_params, pretrained_param_names = [], []
        fixed_params_names = []
        stylizer_params_counts = 0
        for name, param in self.named_parameters():
            if not param.requires_grad:
                continue
            
            # NOTE: need to use flag to choose learnable params
            if self.encoder.stylized:
                if "stylizer.dec" in name or "gaussian_appearance_head" in name:
                    new_params.append(param)
                    new_param_names.append(name)
                    if "stylizer.dec" in name:
                        stylizer_params_counts += 1
                # only finetune style encoder
                elif "stylizer.enc" in name or "stylizer.mask_token" in name or "stylizer.patch_embed" in name:
                    pretrained_params.append(param)
                    pretrained_param_names.append(name)
                    stylizer_params_counts += 1
                else:
                    # not tracking operations for frozen parts to save memory
                    param.requires_grad = False
                    fixed_params_names.append(name)
            else:
                if not self.distill_only:
                    if  "stylizer.dec" in name or "gaussian_appearance_head" in name or "gaussian_param_head" in name or "intrinsic_encoder" in name:
                        new_params.append(param)
                        new_param_names.append(name)
                    else:
                        pretrained_params.append(param)
                    ...
                    if  "structure_builder" in name or "downstream_head" in name:
                        new_params.append(param)
                        new_param_names.append(name)
        ...
        print("Number of params in token stylizer:", stylizer_params_counts)
        print("Optimizing these params:", new_param_names)
