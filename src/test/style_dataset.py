...
# NOTE: only for overftting test, should not be used in real training
...
class PreprocessDataset(Dataset):
    def __init__(self, style_dir, transforms=trans):
        ...
        for i in tqdm(os.listdir(source_dir)):
            ...
            try:
                ...
                if len(image.shape) == 3 and image.shape[-1] == 3:
                    ...
                    io.imsave(os.path.join(target_dir, filename), np.uint8(image * 255))
            except:
                continue
    ...
    def __getitem__(self, index):
        style_image_path = self.images[index]
