...
def render_video_generic(
    ...
    for key, value in visualizations.items():
        ...
        clip.write_videofile(str(output_dir / f"{filename}.mp4"),
                             logger=None)


def render_video_interpolation(stylized_gaussians, decoder,
    ...
    return render_video_generic(stylized_gaussians, decoder, batch,
                                trajectory_fn, "rgb", device, output_dir, filename)
...
def infer(cfg_dict: DictConfig):
    ...
    # data_dir = Path('colmap_test_data/scenes/horns')
    ...
    # data_dir = Path('/ssdwork/datasets/tnt/scene_256/scene_256/truck')
    # data_dir = Path('/ssdwork/datasets/tnt/scene/truck')

    style_dir = Path('/ssdwork/datasets/stylerf_style')
    # Get all .jpg files in the directory (non-recursive)
    style_images = style_dir.glob('*.jpg')
    
    ctx_indices = torch.tensor([5, 9])
    num_ctx_views = 3
    # ----------------------------------------------------------------------------------------------
    ...
    all_style_images = []
    all_style_images_name = []
    style_images_count = 0
    for specified_style_image_path in style_images:
        ...
        all_style_images.append(style_image)
        all_style_images_name.append(style_image_name)
        style_images_count += 1
        
    print(f"There are {style_images_count} style images in total!")
    ...
    example = {
        ...
        "scene": scene,
        "style": {
            "image": all_style_images,
            "image_name": all_style_images_name
        },
    }
    ...
    for i in tqdm(range(style_images_count)):
        style = {'image': batch["style"]["image"][i]}
        style_name = batch['style']['image_name'][i][:-4]
        with torch.no_grad():
            stylized_gaussians = encoder(batch["context"], style)
        
        # render interpolated video with non-stylized and stylized Gaussians
        render_video_interpolation(stylized_gaussians, decoder, batch,
                                device, output_dir, style_name)
