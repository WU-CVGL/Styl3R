...
from src.visualization.camera_trajectory.interpolation import (
    ...
    interpolate_poses_with_timestamps
)
...
def render_video_interpolation(stylized_gaussians, decoder,
    ...
    def trajectory_fn(t):
        ...
        #     t,
        # )
        extrinsics = interpolate_poses_with_timestamps(batch["context"]["extrinsics"][0], t)
...
def infer(cfg_dict: DictConfig):
    ...
    # use the root in cfg
    data_cfg = cfg.dataset[0].dl3dv_style
    root = data_cfg.roots[0]

    # ---------------------------------------------------------------------------------------------
    # TODO: specify scene_idx, style_image_path, left and rightmost ctx view indices, num of views
    scene_idx = '8K/0bfdd020cf475b9c68e4b469d1d1a2d0cad303eefe8b78fb2307855afdaac8be'
    scene_idx = '032dee9fb0a8bc1b90871dc5fe950080d0bcd3caf166447f44e60ca50ac04ec7'
    scene_idx = 'ff592398657b3dfe94153332861985194a3e3c9d199c4a3a27a0ce4038e81ade'

    # specified_style_image_path = Path('colmap_test_data/styles/umbrella_girl.jpg')
    ...
    ctx_indices = torch.tensor([6, 26, 34, 42, 50, 57, 65, 74, 79, 84, 92, 99])
    ctx_indices = torch.tensor([6, 26, 34, 42, 50, 57, 60, 65, 70, 74])
    ctx_indices = torch.tensor([0, 19, 29, 33, 39, 49])
    ctx_indices = torch.tensor([0, 3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33])
    num_ctx_views = 12
    # ----------------------------------------------------------------------------------------------
    ...
    #     3, *data_cfg.original_image_shape)
    assert not (data_cfg.skip_bad_shape and context_image_invalid), "Images don't have the right shape!"
    ...
    with torch.no_grad():
        ...
        stylized_gaussians = encoder(batch["context"], batch["style"], visualization_dump=visualization_dump)
