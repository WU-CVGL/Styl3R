...
def infer(cfg_dict: DictConfig):
    ...
    # merge hydra overrides into typed config
    root_cfg = load_typed_root_config(cfg_dict)
    root_cfg.scene_name = cfg_dict.scene_name
    root_cfg.frame_ids  = cfg_dict.frame_ids
    root_cfg.style_id   = cfg_dict.style_id
    cfg = root_cfg
    ...
    output_dir = Path("outputs_render")
    ...
    data_dir = Path('datasets/TNT/{}'.format(scene_name))

    specified_style_image_path = Path('datasets/styles/{}.jpg'.format(style_id))
    
    ctx_indices = torch.tensor(frame_ids)
    # ----------------------------------------------------------------------------------------------
    ...
    target_indices = ctx_indices.clone()
    print(f"Using target_indices = {target_indices}")
    ...
    # --- continuous naming across frame groups ---
    style_tag = str(style_id)         # e.g. "00"
    save_root = output_dir / scene / style_tag
    save_root.mkdir(parents=True, exist_ok=True)

    # each group has exactly 4 frames → use true frame_ids for numbering
    for idx_in_group, color in enumerate(stylized_output.color[0]):

        true_frame_id = frame_ids[idx_in_group] + 1     # 0→1, 1→2, ..., 15→16
        filename = f"{true_frame_id:06d}.png"

        save_image(color, save_root / filename)
